...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
                                                ...
                                                <th class="border-bottom-0">Condition</th>
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['med_id']; ?>" type="checkbox"  />			
											<span class="custom-control-label"></span>
										</label>
									</td>
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['med_id']; ?>"><?php echo $row['med_title']; ?></a>
											</div>
										</div>
									</td>
                                    <td>
                                    		 <?php if ($row['med_image']!="") { ?>
                                    <img src="<?php echo URL; ?>classes/timthumb.php?src=<?php echo URL ?>images/medication/<?php echo $row['med_image']; ?>&w=100&h=100&zc=1">
                                    <?php } ?>
                                    </td>
                                     <td><?php echo getConditionName_multi($row['med_conditions']); ?></td>
									<td class="align-middle">
										<div class="btn-group align-top">
											<button class="btn btn-sm btn-white"  ><a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['med_id']; ?>">Edit</a></button>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Medication : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
...
<div class="row">
   ...
   <div class="card-body pb-2">
							...
							<div class="form-group">
								...
								<input class="form-control mb-4" type="text" name="txtTitle" value="<?php echo $row['med_title']?>" required>
							</div>
	...
	<select multiple="multiple" name="ckConditions[]" class="multi-select">
     ...
     <?php
										...
										foreach ($results as $value)
										 {
											 ...
											 if ($row['med_conditions']!="")
											{
												$strCategory=explode(",",$row['med_conditions']);
											}
											?>
										<option value="<?php echo $value['condition_id']; ?>" <?php if ($_GET['task']=="edit") { if (@in_array($value['condition_id'],$strCategory)) echo 'selected'; } ?>><?php echo $value['condition_title']; ?></option>
										 <?php
                                        }
                                        ?>
					   ...
					   <!-- Image Upload -->
                            ...
                            <div class="form-group">
								<label class="form-label">Highlighted bullet points</label>
								<textarea class="summernote"  name="txtHighlights" rows="6" cols="150" ><?php echo $row['med_highlights']?></textarea>
							</div>
                           
                            <div class="form-group">
								<label class="form-label">Description</label>
								<textarea class="summernote"  name="txtDescription" rows="6" cols="150" ><?php echo $row['med_description']?></textarea>
							</div>
                            
                             <div class="form-group">
								<label >Directions</label>
								<textarea class="summernote"  name="txtDirections" rows="6" cols="150" ><?php echo $row['med_directions']?></textarea>
							</div>
                            
                             <div class="form-group">
								<label class="form-label" rows="6" cols="150">Side Effects</label>
								<textarea class="summernote"  name="txtSideEffects" rows="6" cols="150" > <?php echo $row['med_side_effects']?></textarea>
							</div>
                            
                             <div class="form-group">
								<label class="form-label">Warnings</label>
								<textarea class="summernote"  name="txtWarnings" rows="6" cols="150" > <?php echo $row['med_warnings']?></textarea>
							</div>
						...
						<div class="custom-controls-stacked">
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="1" <?php if($row['med_status']=="1" || $row['med_status']=='') echo 'checked="checked"'; ?>>
								<span class="custom-control-label">Yes</span>
							</label>
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="0" <?php if($row['med_status']==0 && $row['med_status']!='') echo 'checked="checked"'; ?>>
...
<input type="hidden" name="pageId" value="<?php echo $row['med_id']?>" />	
