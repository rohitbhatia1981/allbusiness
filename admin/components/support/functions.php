<?php
	...
	global $database, $page, $pagingObject;
		...
		if($_GET['txtSearch'] != "")
		{
			$sql .= " and (message_subject like '%".$database->filter($_GET['txtSearch'])."%' || message_text like '%".$database->filter($_GET['txtSearch'])."%') ";
	...
	$fileCount = count($_FILES['flDoc']['name']);
		...
		$updateStatus="update tbl_tickets set message_replier_status=1 where message_id='".$database->filter($hid)."'";
