...
function showList() {
    ...
    $sql = "SELECT * FROM tbl_broker_request WHERE 1";
	
	if ($_GET['ty'] != "") {
        $sql .= " AND br_status='".$database->filter($_GET['ty'])."'";
    }

    if ($_GET['txtSearchByTitle'] != "") {
        $sql .= " AND (br_firstname LIKE '%" . $database->filter($_GET['txtSearchByTitle']) . "%' OR br_lastname LIKE '%" . $database->filter($_GET['txtSearchByTitle']) . "%' OR br_email LIKE '%" . $database->filter($_GET['txtSearchByTitle']) . "%' OR br_company LIKE '%" . $database->filter($_GET['txtSearchByTitle']) . "%')";
    }
