<?php
	...
	global $database, $page, $pagingObject;
		...
		//$sql = "SELECT * FROM tbl_faq_condition where 1 order by page_title asc";


		$sql = "SELECT * FROM tbl_faq_condition where 1";
	...
	global $database, $component;
		...
		$faq_category = $_POST['faq_conditions'];

		

		$_SESSION['sessFAQCondition']=$faq_category;
		...
		$add_query = $database->insert( 'tbl_faq_condition', $names );
	...
	function createFormForPages($id)

			{
				...
				$sql = "SELECT * FROM tbl_faq_condition where faq_id='".$database->filter($id)."'";
...
//Add the WHERE clauses
		...
		$updated = $database->update( 'tbl_faq_condition', $update, $where_clause, 1 );
...

	{
...

		{
			...
			$delete = $database->delete( 'tbl_faq_condition', $where_clause, 1 );
...

	{
		...
		global $database,$component;	
			...
			$del = $database->delete( 'tbl_faq_condition', $where_clause, 1 );
			print "<script>window.location='index.php?c=".$component."&Cid=9'</script>";
