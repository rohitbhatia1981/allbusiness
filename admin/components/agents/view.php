...
// Fetch categories from the database
	$sqlCategories = "SELECT member_tradingname,member_id FROM tbl_members WHERE member_type=1 and member_tradingname<>'' ORDER BY member_tradingname asc";
...
<select name="cmbAgency" class="form-control " data-placeholder="All">
    ...
    <?php if (count($resCategories)>0) {
		...
		for ($j=0;$j<count($resCategories);$j++)
		{
		 ...
		 ?>
    <option value="<?php echo $rowCategories['member_id'] ?>"><?php echo $rowCategories['member_tradingname'] ?></option>
    <?php }
	}?>
</select>
