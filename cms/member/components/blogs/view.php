...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
												...
												<th class="border-bottom-0 w-20">Published</th>
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['id']; ?>" type="checkbox" />			
									...
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												...
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['id']; ?>"><?php echo $row['blog_title']; ?></a>
											</div>
										</div>
									</td>
									<td class="align-middle">
					...
					<td><?php echo date("m/d/Y", strtotime($row['blog_add_date'])); ?></td>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Blog : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
...
<div class="row">
   ...
   <div class="card-body pb-2">
						  ...
						  <div class="form-group">
								...
								<input class="form-control mb-4" type="text" id="blog_title" name="blog_title" value="<?php echo $row['blog_title']?>" required>
							</div>
					

							<div class="form-group">
								<label class="form-label">Short Description</label>
								<textarea class="form-control mb-4" name="short_description" id="short_description" required><?php echo $row['short_description']?></textarea>
							</div>


							<div class="form-group">
								<label class="form-label">Description</label>
								<textarea class="summernote" name="blog_description" required><?php echo $row['blog_description']?></textarea>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-6">
									<select class="form-control" name="txtCategories" id="txtCategories" >
										<option label="Select Categories"></option>
										<?php
				$query = "SELECT * FROM tbl_blog_categories where categories_status=1 order by categories_name";
				$results = $database->get_results( $query );
						...
						foreach ($results as $value) {
								...
								<option value="<?php echo $value['categories_id']; ?>"  <?php if($row['blog_categories'] == $value['categories_id']) {	echo 'selected="selected"';}?>  ><?php echo $value['categories_name']; ?></option>
							...
							<div class="form-group">
								...
								<textarea class="form-control mb-4" name="blog_meta_title" id="blog_meta_title" rows="3" ><?php echo $row['blog_seo_title']?></textarea> 
							</div>
							
							
							<div class="form-group">
								<label class="form-label">SEO Meta Keywords</label>
								<textarea class="form-control mb-4" name="blog_meta_keywords" id="blog_meta_keywords" rows="3" ><?php echo $row['blog_seo_keywords']?></textarea> 
							</div>

							<div class="form-group">
								<label class="form-label">SEO Meta Description</label>
								<textarea class="form-control mb-4" name="blog_meta_description" id="blog_meta_description" rows="3" ><?php echo $row['blog_seo_description']?></textarea> 
							</div>
					   ...
					   <!-- Image Upload -->
						...
						<div class="custom-controls-stacked">
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="1" <?php if($row['blog_status']=="1" || $row['blog_status']=='') echo 'checked="checked"'; ?>>
								<span class="custom-control-label">Yes</span>
							</label>
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="0" <?php if($row['blog_status']==0 && $row['blog_status']!='') echo 'checked="checked"'; ?>>
