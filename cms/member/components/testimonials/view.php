...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
												...
												<th width="185"   class="border-bottom-0 w-60">Client</th>
                                                <th width="268" class="border-bottom-0 w-60">Testimonial</th>
												<th width="63" class="border-bottom-0 w-20">Actions</th>
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['testimonial_id']; ?>" type="checkbox" />			
											<span class="custom-control-label"></span>
										</label>
									</td>
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												...
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['testimonial_id']; ?>"><?php echo $row['testimonial_client']; ?></a>
											</div>
										</div>
									</td>
                                    <td><?php echo substr(strip_tags(fnUpdateHTML($row['testimonial_text'])),0,80); ?>..</td>
									<td class="align-middle">
									...
									<td class="align-middle">
										...
										<?php }else if($row['testimonial_status'] == 0){ ?>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Testimonial : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
...
<div class="row">
   ...
   <div class="card-body pb-2">
							...
							<div class="form-group">
								...
								<input class="form-control mb-4" type="text" id="txtClient" name="txtClient" value="<?php echo $row['testimonial_client']?>" required>
							</div>
							...
							<div class="form-group">
								...
								<textarea class="summernote" rows=5 cols="80" name="txtDescription" required><?php echo $row['testimonial_text']?></textarea>
							</div>
						...
						<!-- Image Upload -->
                        	...
                        	<div class="form-group">
								...
								<input class="form-control mb-4" type="text" id="txtDesignation" name="txtDesignation" value="<?php echo $row['testimonial_designation']?>">
							</div>
					   ...
					   <!-- Image Upload -->
						...
						<div class="custom-controls-stacked">
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="1" <?php if($row['testimonial_status']=="1" || $row['testimonial_status']=='') echo 'checked="checked"'; ?>>
								<span class="custom-control-label">Yes</span>
							</label>
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="0" <?php if($row['testimonial_status']==0 && $row['testimonial_status']!='') echo 'checked="checked"'; ?>>
...
<input type="hidden" name="id" value="<?php echo $row['testimonial_id']?>" />	
			...
			<?php if ($row['testimonial_image']!="")
			 $pImageStr="'".$row['testimonial_image']."'";		 
			  ?>
...
$(document).ready(function(){
	$('#images4ex').orakuploader({
		...
		orakuploader_main_path : '../images/testimonials',
		orakuploader_thumbnail_path : '../images/testimonials',
