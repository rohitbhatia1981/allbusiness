...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
					...
					<div class="e-panel card">
						<div class="card-body">
							...
							<div class="row">
											...
											<div class="col-md-12 col-lg-12 col-xl-3">
												<div class="form-group">
													<label class="form-label">Filter by Page Category:</label>
                                                    
                                                    <?php
														$sqlCategory="select * from tbl_page_categories where page_categories_status=1 order by page_categories_name";
													...
													<select name="cmbCategory"  class="form-control custom-select select2" data-placeholder="All Category">
                                                        ...
                                                        <?php for ($c=0;$c<count($resCategory);$c++)
														{
															...
															?>
														<option value="<?php echo $rowCategory['page_categories_id']?>" <?php if ($rowCategory['page_categories_id']==$_GET['cmbCategory']) echo "selected"; ?>><?php echo $rowCategory['page_categories_name']?></option>
														<?php } ?>
													</select>
												</div>
											</div>
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
												...
												<th width="8%" class="border-bottom-0 w-60">Title</th>
                                                <th width="12%" class="border-bottom-0 w-60">Page Category</th>
                                                <th width="26%" class="border-bottom-0 w-60">Page Content</th>
												
												<th width="19%" class="border-bottom-0 w-20">Actions</th>
												<th width="26%" class="border-bottom-0 w-20">Status</th>
											</tr>
										</thead>
							<?php
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['page_id']; ?>" type="checkbox"  />			
									...
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['page_id']; ?>"><?php echo $row['page_title']; ?></a>
											</div>
										</div>
									</td>
                                    <td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<?php echo $row['page_categories_name']; ?>
											</div>
										</div>
									</td>
                                    ...
                                    <td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<?php echo substr(strip_tags(fnUpdateHTML($row['page_description'])),0,50); ?>...
											</div>
										</div>
									</td>
									<td class="align-middle">
										<div class="btn-group align-top">
											<button class="btn btn-sm btn-white"  ><a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['page_id']; ?>">Edit</a></button>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Page : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
...
<div class="row">
   ...
   <div class="card-body pb-2">
					...
					<div class="form-group">
								...
								<input class="form-control mb-4" type="text" name="page_title" value="<?php echo $row['page_title']?>" required>
							</div>
                            
                            <div class="form-group">
								<label class="form-label">Page Categories</label>
							
									<select class="form-control" name="txtCategories" id="txtCategories" required >
										<option label="Select Categories"></option>
										<?php
				$query = "SELECT * FROM tbl_page_categories where page_categories_status=1 order by page_categories_name";
				$results = $database->get_results( $query );
						...
						foreach ($results as $value) {
								...
								<option value="<?php echo $value['page_categories_id']; ?>"  <?php if($row['page_categories'] == $value['page_categories_id']) {	echo 'selected="selected"';}?>  ><?php echo $value['page_categories_name']; ?></option>
							...
							<div class="form-group">
                                ...
                                <textarea name="page_description" class="summernote"><?php echo fnUpdateHTML($row['page_description'])?></textarea>
								<!--<textarea class="content" name="page_description" required><?php echo $row['page_description']?></textarea>-->
							</div>
