...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['page_categories_id']; ?>" type="checkbox" />			
											<span class="custom-control-label"></span>
										</label>
									</td>
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['page_categories_id']; ?>"><?php echo $row['page_categories_name']; ?></a>
											</div>
										</div>
									</td>
									<td class="align-middle">
										<div class="btn-group align-top">
											<button class="btn btn-sm btn-white"  ><a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['page_categories_id']; ?>">Edit</a></button>
									...
									<td class="align-middle">
										<div class="btn-group align-top">
										<?php if($row['page_categories_status'] == 1){ ?>

										<span class="tag tag-green">Enabled</span>

										<?php }else if($row['page_categories_status'] == 0){ ?>
...
?>
	...
	<?php 
	$blog_image_display = $row['page_categories_banner_image'];
	echo $blog_image_display; 
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Page Category : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
...
<div class="row">
   ...
   <div class="card-body pb-2">
					...
					<div class="form-group">
								...
								<input class="form-control mb-4" type="text" name="category_title" id="category_title" value="<?php echo $row['page_categories_name']?>" required>
							</div>

							<div class="form-group">
								<label class="form-label">SEO Meta Title</label>
								<textarea class="form-control mb-4" name="page_meta_title" id="page_meta_title" rows="3" ><?php echo $row['page_categories_seo_title']?></textarea> 
							</div>
							
							
							<div class="form-group">
								<label class="form-label">SEO Meta Keywords</label>
								<textarea class="form-control mb-4" name="page_meta_keywords" id="page_meta_keywords" rows="3" ><?php echo $row['page_categories_seo_keywords']?></textarea> 
							</div>

							<div class="form-group">
								<label class="form-label">SEO Meta Description</label>
								<textarea class="form-control mb-4" name="page_meta_description" id="page_meta_description" rows="3"><?php echo $row['page_categories_description']?></textarea> 
							</div>
						...
						<div class="custom-controls-stacked">
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="1" <?php if($row['page_categories_status']=="1" || $row['page_categories_status']=='') echo 'checked="checked"'; ?>>
								<span class="custom-control-label">Yes</span>
							</label>
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="0" <?php if($row['page_categories_status']==0 && $row['page_categories_status']!='') echo 'checked="checked"'; ?>>
...
<input type="hidden" name="page_categories_id" value="<?php echo $_GET['id']?>" />

<input type="hidden" name="parentgroupId" value="<?php echo $_SESSION['sess_member_groupid']?>" />

<input type="hidden" name="parentuserId" value="<?php echo $_SESSION['user_id']?>" />
	</form>					
								</div>
			<?php if ($row['page_categories_banner_image']!="")
			 $pImageStr="'".$row['page_categories_banner_image']."'";		 
			  ?>
...
$(document).ready(function(){
	$('#images4ex').orakuploader({
		...
		orakuploader_main_path : '../images/page_banner',
		orakuploader_thumbnail_path : '../images/page_banner',
