...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
												...
												<th class="border-bottom-0 w-70">Parent Category</th>
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['categories_id']; ?>" type="checkbox"  />			
									...
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['categories_id']; ?>"><?php echo $row['categories_name']; ?></a>
											</div>
										</div>
									</td>
                                    <td>
                                    ...
                                    <?php
									if ($row['categories_parent_name']!="")
									echo $row['categories_parent_name'];
									else
									echo "-";
									?>
									...
									<td class="align-middle">
										<div class="btn-group align-top">
											<button class="btn btn-sm btn-white"  ><a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['categories_id']; ?>">Edit</a></button>
									...
									<td class="align-middle">
										<div class="btn-group align-top">
										<?php if($row['categories_status'] == 1){ ?>

										<span class="tag tag-green">Enabled</span>

										<?php }else if($row['categories_status'] == 0){ ?>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Professional Category : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
   ...
   <div class="card-body pb-2">
					...
					<div class="form-group">
                   		 <div class="col-6">                    
					 	 	...
					 	 	<input class="form-control mb-4" type="text" name="conditioncat_title" id="conditioncat_title" value="<?php echo $row['categories_name']?>" required>
                       	</div>
					</div>
    ...
    <div class="col-6">
        <select class="form-control" name="cmbParent" id="cmbParent">
            <option label="Select Parent Category (if any)"></option>
            <?php
            $query = "SELECT * FROM tbl_categories where categories_status = 1 and categories_parent_id=0 ORDER BY categories_name";
            $results = $database->get_results($query);

            foreach ($results as $value) {
                ?>
                <option value="<?php echo $value['categories_id']; ?>" <?php if ($row['categories_parent_id'] == $value['categories_id']) {
                    echo 'selected="selected"';
                } ?> ><?php echo $value['categories_name']; ?></option>
                <?php
            }
            ?>
        </select>
						...
						<div class="custom-controls-stacked">
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="1" <?php if($row['categories_status']=="1" || $row['categories_status']=='') echo 'checked="checked"'; ?>>
								<span class="custom-control-label">Yes</span>
							</label>
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="0" <?php if($row['categories_status']==0 && $row['categories_status']!='') echo 'checked="checked"'; ?>>
...
<input type="hidden" name="conditionCat_id" value="<?php echo $row['categories_id']?>" />	
