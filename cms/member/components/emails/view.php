...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['email_id']; ?>" type="checkbox"  />			
											<span class="custom-control-label"></span>
										</label>
									</td>
									<td class="align-middle">
										<div class="d-flex">
											<div class="ml-3 mt-1">
												<a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['email_id']; ?>"><?php echo $row['email_title']; ?></a>
											</div>
										</div>
									</td>
									<td class="align-middle">
										<div class="btn-group align-top">
											<button class="btn btn-sm btn-white"  ><a href="?c=<?php echo $component?>&task=edit&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['email_id']; ?>">Edit</a></button>
											

<?php //if($permission['rights_delete'] == 1) { ?>

		<!--<a href="?c=<?php echo $component?>&task=deleted&Cid=<?php echo $menuid['component_headingid']; ?>&id=<?php echo $row['email_id']; ?>"><i class="btn btn-sm btn-white"><span class="feather feather-trash-2 text-muted fs-16"></span></i></a>-->
...
<?php //} ?>
									...
									<td class="align-middle">
										...
										<?php }else if($row['email_status'] == 0){ ?>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">Emails : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
...
<div class="row">
   ...
   <div class="card-body pb-2">
							...
							<div class="form-group">
								...
								<textarea class="form-control" rows="10" cols="100"  name="page_description" required><?php echo $row['email_description']?></textarea>
							</div>
						...
						<div class="custom-controls-stacked">
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="1" <?php if($row['email_status']=="1" || $row['email_status']=='') echo 'checked="checked"'; ?>>
								<span class="custom-control-label">Yes</span>
							</label>
							<label class="custom-control custom-radio">
								<input type="radio" class="custom-control-input" name="rdoPublished" id="rdoPublished" value="0" <?php if($row['email_status']==0 && $row['email_status']!='') echo 'checked="checked"'; ?>>
