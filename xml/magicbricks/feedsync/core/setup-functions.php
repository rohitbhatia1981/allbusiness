...
function init_db_connection() {
    ...
    if( !defined('DB_USER')  || !defined('DB_PASS')  || !defined('DB_NAME')  || !defined('DB_HOST')  ) {
        ...
        if ( ! isset($feedsync_db->dbh) || ! $feedsync_db->dbh ) {

            $feedsync_db->connect($feedsync_db->dbuser, $feedsync_db->dbpassword, $feedsync_db->dbhost, $feedsync_db->dbport);
            $feedsync_db->select($feedsync_db->dbname,$feedsync_db->encoding);

        }

        // set connection charset
        set_charset($feedsync_db->dbh);
...
function get_db_version() {
    global $feedsync_db;
    $server_info = mysqli_get_server_info( $feedsync_db->dbh );
...
function fix_charset_collate() {
    ...
    if( version_compare( $current_version, '3.2.3', '<=' ) && get_option('chartset_collation_updated') != 'yes' ){
        
        $charset_collate = init_charset();

        $tables = $feedsync_db->get_col('SHOW TABLES');
        
        if( !empty($tables) ) {

            foreach($tables as $table) {
                $feedsync_db->query("ALTER TABLE $table CONVERT TO CHARACTER SET ".$charset_collate['charset']." COLLATE ".$charset_collate['collate']);
            }
        }
