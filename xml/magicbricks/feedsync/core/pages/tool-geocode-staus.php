...
*/
	...
	$geocode = file_get_contents("https://maps.google.com/maps/api/geocode/json?address=$addr&sensor=false&key=".get_option('feedsync_google_api_key') );
		...
		<p><?php echo $input_addr; ?></p>
			...
			<div class="button-margin" style="margin: 2em 0;">
				...
				if ($result['status'] == 'OK') { ?>
						...
						<span  class="label label-success">Geocode Successful!</span>

				<?php
				} else { ?>
					<span  class="label label-warning">Warning :( <?php echo $result['status']; ?></span>
				...
				if( get_option('feedsync_google_api_key') == '') {
					?>
					<span class="label label-warning">Please provide Google MAP API key for generating listing coordinates on settings page.</span>
					<?php
				}
